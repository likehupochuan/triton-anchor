...
未设置仓库变量时，生产环境使用以下默认值：

```text
LOCAL_CI_FALLBACK_WORKER_BRANCH=CI_dev
LOCAL_CI_PAGES_BRANCH=CI_dev
DASHBOARD_SOURCE_BRANCH=ci/push/CI_dev
DASHBOARD_FULL_TEST_SOURCE_BRANCH=ci/full/CI_dev
```

仓库 Variables 可以覆盖这些默认值。部署前应确认没有遗留的测试分支配置；Gitee owner、仓库、URL、认证用户名和结果分支继续使用部署环境的现有配置。
