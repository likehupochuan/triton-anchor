...
- Codex 连接异常在尝试次数小于上限时不告警；第 10 次仍在执行也不告警，只有第 10 次结束后仍连接失败才报告。认证、限流和其他 Codex 错误按各自状态独立判断。
- `result_missing`、`cli_failed` 等 Worker 已在预算内自动处理的恢复过程不单独告警；恢复预算用尽后再按最终状态报告。
